...
Même architecture que SensorGuard v3 — noyau INDUSTRIA commun
"""
import numpy as np
...
class LSTMAutoencoder(nn.Module):
    ...
    def forward(self, x):
        h, c = self.encoder(x); return self.decoder(h, c)

class InfraGuardDetector:
    def __init__(self, window=96, hidden=48, epochs=25,
                 threshold_pct=95.0, batch_size=128, lr=1e-3, step=4):
        self.window=window; self.hidden=hidden; self.epochs=epochs
        self.threshold_pct=threshold_pct; self.batch_size=batch_size
        self.lr=lr; self.step=step
        self.scaler=MinMaxScaler((-1,1)); self.model=None
        self.threshold=None; self._losses=[]; self._fitted=False

    def _windows(self, data):
        T,D=data.shape; idx=np.arange(0,T-self.window+1,self.step)
        return np.stack([data[i:i+self.window] for i in idx])

    def fit(self, train, progress_cb=None):
        norm=self.scaler.fit_transform(train)
        W=self._windows(norm); D=norm.shape[1]
        Xt=torch.tensor(W,dtype=torch.float32)
        loader=DataLoader(TensorDataset(Xt,Xt),batch_size=self.batch_size,shuffle=True)
        self.model=LSTMAutoencoder(D,self.hidden,window=self.window)
        opt=torch.optim.Adam(self.model.parameters(),lr=self.lr)
        crit=nn.MSELoss()
        self.model.train(); self._losses=[]
        for ep in range(self.epochs):
            ep_loss=0.0
            for xb,yb in loader:
                opt.zero_grad(); out=self.model(xb); loss=crit(out,yb)
                loss.backward(); opt.step(); ep_loss+=loss.item()*len(xb)
            ep_loss/=len(Xt); self._losses.append(ep_loss)
            if progress_cb: progress_cb(ep+1,ep_loss)
    ...
    def _score(self, norm):
        T,D=norm.shape; W_arr=self._windows(norm)
        Xt=torch.tensor(W_arr,dtype=torch.float32)
        errors=np.zeros((len(Xt),self.window))
        self.model.eval()
        with torch.no_grad():
            for i in range(0,len(Xt),256):
                b=Xt[i:i+256]; rec=self.model(b).numpy()
                orig=W_arr[i:i+256]
                errors[i:i+256]=((orig-rec)**2).mean(axis=2)
        score=np.zeros(T); count=np.zeros(T)
        for i,err in enumerate(errors):
            s=i*self.step; e=s+self.window
            score[s:e]+=err; count[s:e]+=1
    ...
    def predict(self, test):
        assert self._fitted
        norm=self.scaler.transform(test); scores=self._score(norm)
        preds=(scores>=self.threshold).astype(int)
        return {"scores":scores,"predictions":preds,
                "threshold":self.threshold,"losses":self._losses}
